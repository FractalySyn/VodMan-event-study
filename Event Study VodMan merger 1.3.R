...
event_study = function(est_data, event_data, events, model = "capm", bootstrap_multiplier)
{
  ...
  if (model == "capm") ARs = market_model(est_data, est_data[,1], est_data[,2])[[1]]$residuals %>% abs()
  if (model == "cmrm") ARs = (est_data[,1] - mean(est_data[,1])) %>% abs()
  if (model == "b-one") ARs = (est_data[,1] - est_data[,2]) %>% abs()
  
  mc_ar = sample(ARs, bootstrap_multiplier, replace = T)
  ...
  {
    a = abs(car[n]) / length(indexes[[n]])
    pvalue_montecarlo_test[n] = mean(mc_ar > a)
  }
  ...
  plot(density(bs_firm))
  plot(density(est_data[,1]))
